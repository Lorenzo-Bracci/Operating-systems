...
struct head *flists[16];

int getLocation(int size){
  int location = round(((size - 16) / 8));
  if(location > 15){
    location = 15;
  }
  return location;
...
int done = FALSE;
struct head *current = flists[getLocation(block->size)];
...
void detach(struct head *block) {
  struct head *flist = flists[getLocation(block->size)];
if(block == flist){
  flists[getLocation(block->size)] = block->next;
}
  if (block->next != NULL){
...
void insert(struct head *block) {
  ...
  block->prev = NULL;
  if (flist != NULL){
    flists[getLocation(block->size)]->prev = block;
  }
  flists[getLocation(block->size)] = block;
}
...
struct head *current = flists[location];
  if(current == NULL){
  current = flists[15];
  }
